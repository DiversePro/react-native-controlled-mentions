...
const MentionInput = (_a) => {
    var { value, onChange, partTypes = [], inputRef: propInputRef, containerStyle, onSelectionChange,onLayout,setTriggerActive } = _a, textInputProps = __rest(_a, ["value", "onChange", "partTypes", "inputRef", "containerStyle", "onSelectionChange","onLayout",'setTriggerActive']);
    const textInput = react_1.useRef(null);
    const selectionRef = react_1.useRef({
        start: 0,
        end: 0,
      });
    ...
    const keywordByTrigger = react_1.useMemo(() => {
        ...
        let val = utils_1.getMentionPartSuggestionKeywords(parts, plainText, selectionRef.current, partTypes);
        let triggerActive = partTypes.some((item,index)=>val[item.trigger] != undefined);
        setTriggerActive(triggerActive);
        return val;
        
    }, [parts, plainText, selectionRef.current, partTypes]);
    /**
